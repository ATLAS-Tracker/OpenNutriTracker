...
class _AddWeightScreenState extends State<AddWeightScreen> {
  ...
  Future<WeightSummary> _getWeightsSummary() async {
    ...
    final List<WeightData> weightDataList = List.generate(nbDays, (i) {
      final date =
          DateTime(_day.year, _day.month, _day.day).subtract(Duration(days: i));
      final weight = weightsMap[date];
      return WeightData(date, weight);
    }).reversed.toList(); // Reverse to have dates in chronological order.

    final averageWeight =
        await _getWeightUsecase.getAverageWeight(_day, nbDays);

    return WeightSummary(
        averageWeight: averageWeight, weightDataList: weightDataList);
  }
              ...
              builder: (BuildContext context,
                ...
                if (snapshot.hasData) {
                  ...
                  final double averageWeight =
                      roundDecimal(weightSummary.averageWeight);
